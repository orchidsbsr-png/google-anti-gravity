// Server-side Brevo email. Uses BREVO_API_KEY (server env) — the key
// never ships to the browser, and failures land in Vercel logs instead
// of vanishing in a customer's console.
...
export async function sendOrderConfirmationEmail(order, customerEmail) {
    if (!BREVO_API_KEY) {
        console.error('❌ BREVO_API_KEY not set — order confirmation not sent');
        return { ok: false, detail: 'BREVO_API_KEY not configured' };
    }
    if (!customerEmail) {
        return { ok: false, detail: 'Order has no customer email' };
    }
    ...
    const orderRef = order.id ? String(order.id).slice(0, 8).toUpperCase() : 'PENDING';
    const customerName = order.customer_details?.name || 'there';
                    ...
                    <div style="max-width: 580px; margin: 0 auto;">
                        ...
                        <div style="background-color: #2D3319; border-radius: 18px 18px 0 0; padding: 40px 36px; text-align: center;">
                            ...
                            <p style="margin: 18px 0 0; font-family: Arial, sans-serif; font-size: 13px; line-height: 1.7; color: rgba(247,244,236,0.75);">
                                Hi ${customerName}, thank you for ordering from our orchard.
    ...
    if (!response.ok) {
        ...
        console.error(`❌ Brevo API error (${response.status}):`, errorText.slice(0, 500));
        return { ok: false, status: response.status, detail: errorText.slice(0, 500) };
    }

    const result = await response.json();
    console.log(`✅ Order confirmation sent to ${customerEmail} (messageId: ${result.messageId || '?'})`);
